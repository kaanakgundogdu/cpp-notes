...
bool operator==(const MyString &lefths, const MyString &righths){

    return (std::strcmp(lefths.str,righths.str)==0);
}


std::ostream &operator<<(std::ostream &os,const MyString &righths){
    os<<righths.str;
    return os;
}
std::istream &operator>>(std::istream &in,MyString &righths){
    char *buff=new char[1000];
    in>>buff;
    righths=MyString(buff);
    delete [] buff;
    return in;
}